...
export default function ForgotPasswordStep({ userData, onBack, onSuccess }: ForgotPasswordStepProps) {
  const [step, setStep] = useState<'select_method' | 'verify_code' | 'new_password'>('select_method');
  ...
  const handleVerifyCode = async (e: React.FormEvent) => {
    ...
    try {
      const endpoint = selectedMethod === 'security_code' ? '/api/auth/verify-security-code' : '/api/auth/verify';
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/auth/reset-password', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: userData.id, code, newPassword, method: selectedMethod })
      });
    ...
    <>
      <div className="flex flex-col pt-4">
        ...
        <h1 className="text-3xl md:text-4xl font-medium text-gray-900 leading-tight mb-8">
          {step === 'select_method' && 'Como deseja receber o código?'}
          {step === 'verify_code' && 'Digite o código de verificação'}
          {step === 'new_password' && 'Crie uma nova senha'}
        </h1>
      ...
      <div className="flex justify-center md:justify-end">
        {step === 'select_method' && (
          <div className="bg-white rounded-xl border border-gray-200 shadow-[0_2px_10px_rgba(0,0,0,0.04)] w-full max-w-md overflow-hidden flex flex-col">
            <div className="p-6 border-b border-gray-100">
              <h2 className="text-xl font-medium mb-2">Confirme seu celular</h2>
              <p className="text-sm text-gray-500">Para redefinir sua senha, precisamos confirmar seu número.</p>
            </div>
            {error && <div className="bg-red-50 text-red-600 p-4 text-sm text-center border-b border-red-100">{error}</div>}
            
            <div className="flex flex-col">
              <button 
                onClick={() => handleMethodSelect('security_code')}
                disabled={loading}
              ...
              >
                ...
                <div className="flex-1">
                  <h3 className="text-sm font-medium text-gray-900 mb-1">Código de Segurança</h3>
                  <p className="text-sm text-gray-500">Usar o código de 6 dígitos criado no cadastro.</p>
                </div>
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-500">Enviar código para o telefone terminado em {maskedPhone}.</p>
                </div>
                <ChevronRight size={20} className="text-gray-300 group-hover:text-gray-400 shrink-0 ml-4" />
              </button>
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-500">Enviar código para {maskedEmail}.</p>
                </div>
                <ChevronRight size={20} className="text-gray-300 group-hover:text-gray-400 shrink-0 ml-4" />
              </button>
            </div>
        ...
        {step === 'verify_code' && (
          <div className="bg-white rounded-xl border border-gray-200 shadow-[0_2px_10px_rgba(0,0,0,0.04)] p-8 w-full max-w-md">
            <h2 className="text-xl font-medium mb-2">
              {selectedMethod === 'security_code' ? 'Código de Segurança' : 'Código de 6 dígitos'}
            </h2>
            <p className="text-sm text-gray-500 mb-6">
              {selectedMethod === 'security_code' 
            ...
            <form onSubmit={handleVerifyCode}>
              ...
              <button 
                type="button" 
                onClick={() => { setStep('select_method'); setCode(''); setError(''); }} 
        ...
        {step === 'new_password' && (
          <div className="bg-white rounded-xl border border-gray-200 shadow-[0_2px_10px_rgba(0,0,0,0.04)] p-8 w-full max-w-md">
            <h2 className="text-xl font-medium mb-2">Nova Senha</h2>
            <p className="text-sm text-gray-500 mb-6">
